...
#### 本项目采用的方法
使用 cyclictest 测量各 CPU 核心的调度延迟分布。
...
│ CPU   │ Min     │ Avg     │ Max     │
├───────┼─────────┼─────────┼─────────┤
│ CPU0  │ 10 us   │ 28 us   │ 163 us  │
│ CPU1  │ 12 us   │ 26 us   │ 3195 us │
│ CPU2  │ 12 us   │ 26 us   │ 151 us  │
│ CPU3  │ 11 us   │ 25 us   │ 106 us  │
│ CPU4  │ 7 us    │ 19 us   │ 95 us   │
│ CPU5  │ 6 us    │ 18 us   │ 37 us   │
└───────┴─────────┴─────────┴─────────┘

Histogram Overflow: CPU1 有 2 次溢出 (在 cycle 929, 6919)
```
...
|------|--------|------|
| 平均延迟 | 18-28 us | 正常范围 |
| 最大延迟 | 37-3195 us | CPU1 有异常峰值 |
| CPU4/5 (A72大核) | 18-19 us avg | 延迟最低，适合实时任务 |

**注意**：
- 当前内核配置未确认是否启用 CONFIG_PREEMPT
- 需要对比 CONFIG_PREEMPT_VOLUNTARY 配置才能验证 30% 降低
...
**当前问题**：
- Pipeline 日志只显示 Frame 0（每 100 帧打印一次）
- MQTT 连接反复断开重连（rc=7）
- 视频文件循环可能存在队列阻塞
...
RTSP 服务配置 `enable_rtsp: true`，但 RTSP 端口 8554 未监听。

**原因分析**：
- RTSP 需要视频编码器启动成功
- 当前 pipeline 在推理后帧处理可能阻塞
- MQTT 反复重连影响输出线程
...
| 测试方式 | 平均延迟 | 备注 |
|----------|----------|------|
| 独立测试程序（BGR）| 64.9ms | 原始测试 |
| 独立测试程序（优化后）| 57.7ms | 禁用 npu_upgrade 后 |
| Pipeline 内测量 | **27.5ms** | BGR→RGB + 优化后 |

**重大改进**：Pipeline 内推理延迟从预期 65ms 降至 **27.5ms**，已低于 40ms 目标！
...
| 2 | 推理抖动 (stddev) | < 5ms | 3-5ms | ✅ 达标 |
| 5 | 启动时间 | < 5s | 14.4s | ❌ 未达标 |
| 6 | 存储分析 | 缩减 30% | 待对比 | ⏳ |
| 7 | cyclictest 平均延迟 | 降低 30% | 18-28 us | ⏳ 待对比 |

### 部分/未完成测试

| 实验 | 状态 | 问题 |
|------|------|------|
| 3 编码丢帧率 | ⚠️ 部分 | Pipeline 稳定但队列有阻塞 |
| 4 RTSP 延迟 | ❌ 未测 | RTSP 端口未监听 |
...
| 推理延迟 65ms → 27.5ms | NPU 服务优化 + BGR→RGB |

### 关键成果

1. **推理延迟达标**：27.5ms < 目标 40ms ✅
2. **启动时间优化**：21s → 14s（-32%）
